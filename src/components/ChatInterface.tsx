...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ user, onLogout }) => {
  ...
  const createNewChat = async () => {
    try {
      ...
      const response = await fetch('https://gemini-backend-21wo.onrender.com/api/chat/sessions', {
        method: 'POST',
        headers: {
    ...
    if (window.confirm('Are you sure you want to delete all chat history? This action cannot be undone.')) {
      try {
        ...
        for (const session of sessionsToDelete) {
          await fetch(`https://gemini-backend-21wo.onrender.com/api/chat/sessions/${session.id}`, {
            method: 'DELETE',
            headers,
          });
        }
